...
def shortest_path(grids, start_x, start_y, dst_x, dst_y, start_minute)
  ...
  while work_queue.size > 0
    ...
    if next_x == dst_x && next_y == dst_y
      ...
      all_moves = possible_moves(grids[the_minute + 2], next_x, next_y, )
      all_moves.each do |next_m|
        if !seen.has_key?([next_x, next_y, next_m, the_minute + 1])
#          work_queue.unshift([next_x, next_y, next_m, the_minute + 1 ])        # use this for DFS
          work_queue << [next_x, next_y, next_m, the_minute + 1 ]               # use this for BFS (faster for my input and implementation)
          seen[[next_x, next_y, next_m, the_minute + 1]] = 1
        end
      end
    end
  end
